...
import { bus, sprites, SPRITE_SIZE, APP, hoveredPawnIdStore, hoveredPawnStore } from "../../ui/store";
...
export class SlimeRenderer {
  ...
  private addSprite(sprite: PIXI.Sprite, pawn: Pawn) {
    ...
    container.x = pawn.x * SPRITE_SIZE;
    container.y = pawn.y * SPRITE_SIZE;
    container.height = SPRITE_SIZE;
    container.width = SPRITE_SIZE;
    container.zIndex = LAYERS.PAWN;

    container.interactive = true;
    // handle display of pawn details when user hovers over this sprite
    container.on("mouseout", () => {
      hoveredPawnIdStore.update(() => 0);
      hoveredPawnStore.update(() => ({}));
    });
    container.on("mouseover", () => {
      hoveredPawnIdStore.update(() => pawn.id);
      hoveredPawnStore.update(() => pawn.json());
    });

    sprites.set(pawn.id, container);
