...
class ExampleAICode {           // This line defines the user submited code, the name can be changed freely
  playerId;                     // Do not edit - This is the player ID randomly chosen from 1 or 2
  map = [];                     // Do not edit - This is the map passed to the player's code
  ...
  // determine if the given location if out of bounds of the map
  outOfBounds(x, y) {
  ...
  takeAction(map, id) {     // Do not edit - Game code will pass the current slime pawn's id and a map matrix
    this.map = map;         // Loads the current map for this turn
    ...
    for (let i = 0; i < map.length; i++) {
      for (let j = 0; j < map.length; j++) {
        if (map[i][j] && map[i][j].owner === this.playerId) {
